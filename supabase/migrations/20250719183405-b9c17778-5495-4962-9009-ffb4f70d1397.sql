-- Primeiro, vamos atualizar os planos existentes sem deletar
-- Para evitar problemas com foreign keys

-- Atualizar planos existentes ou inserir novos
WITH existing_plans AS (
  SELECT id, name FROM public.subscription_plans
),
new_plans_data AS (
  SELECT 
    'Plano Básico' as name,
    'Ideal para pequenas empresas com uma filial' as description,
    99.00 as price_monthly,
    10 as max_users,
    1 as max_branches,
    '["Gestão de funcionários", "Relatórios básicos", "Suporte por email"]'::jsonb as features,
    true as is_active
  UNION ALL
  SELECT 
    'Plano Profissional',
    'Perfeito para empresas em crescimento',
    199.00,
    50,
    5,
    '["Gestão de funcionários", "Múltiplas filiais", "Relatórios avançados", "Suporte prioritário"]'::jsonb,
    true
  UNION ALL
  SELECT 
    'Plano Empresarial',
    'Para grandes empresas com múltiplas filiais',
    499.00,
    200,
    20,
    '["Gestão completa", "Múltiplas filiais", "Relatórios personalizados", "API acesso", "Suporte 24/7"]'::jsonb,
    true
  UNION ALL
  SELECT 
    'Plano Personalizado',
    'Plano customizável conforme necessidade',
    0.00,
    9999,
    9999,
    '["Limites personalizáveis", "Recursos sob demanda", "Suporte dedicado"]'::jsonb,
    true
)
INSERT INTO public.subscription_plans (name, description, price_monthly, max_users, max_branches, features, is_active)
SELECT name, description, price_monthly, max_users, max_branches, features, is_active
FROM new_plans_data
WHERE name NOT IN (SELECT name FROM existing_plans)
ON CONFLICT (name) DO UPDATE SET
  description = EXCLUDED.description,
  price_monthly = EXCLUDED.price_monthly,
  max_users = EXCLUDED.max_users,
  max_branches = EXCLUDED.max_branches,
  features = EXCLUDED.features,
  is_active = EXCLUDED.is_active,
  updated_at = NOW();

-- Adicionar campos personalizáveis para planos personalizados
ALTER TABLE public.company_subscriptions 
ADD COLUMN IF NOT EXISTS custom_max_users INTEGER,
ADD COLUMN IF NOT EXISTS custom_max_branches INTEGER,
ADD COLUMN IF NOT EXISTS custom_price NUMERIC;

-- Função para sincronizar limites da empresa com o plano
CREATE OR REPLACE FUNCTION public.sync_company_limits_with_plan()
RETURNS TRIGGER AS $$
DECLARE
  plan_max_users INTEGER;
BEGIN
  -- Buscar limite de usuários do plano (ou usar valor customizado)
  SELECT 
    CASE 
      WHEN NEW.custom_max_users IS NOT NULL THEN NEW.custom_max_users
      ELSE sp.max_users 
    END INTO plan_max_users
  FROM public.subscription_plans sp 
  WHERE sp.id = NEW.plan_id;

  -- Se não encontrou o plano, usar padrão
  IF plan_max_users IS NULL THEN
    plan_max_users := 50;
  END IF;

  -- Atualizar ou inserir limites da empresa baseado no plano
  INSERT INTO public.company_limits (
    company_id, 
    max_users, 
    max_admins, 
    max_managers, 
    max_supervisors
  ) VALUES (
    NEW.company_id,
    plan_max_users,
    1, -- sempre 1 admin
    GREATEST(1, plan_max_users / 10), -- 10% dos usuários podem ser managers
    GREATEST(1, plan_max_users / 5) -- 20% dos usuários podem ser supervisors
  )
  ON CONFLICT (company_id) 
  DO UPDATE SET
    max_users = plan_max_users,
    max_managers = GREATEST(1, plan_max_users / 10),
    max_supervisors = GREATEST(1, plan_max_users / 5),
    updated_at = NOW();

  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

-- Trigger para sincronizar automaticamente quando assinatura for criada/atualizada
DROP TRIGGER IF EXISTS sync_company_limits_trigger ON public.company_subscriptions;
CREATE TRIGGER sync_company_limits_trigger
  AFTER INSERT OR UPDATE OF plan_id, custom_max_users ON public.company_subscriptions
  FOR EACH ROW
  EXECUTE FUNCTION public.sync_company_limits_with_plan();

-- Sincronizar limites existentes das empresas com assinaturas
DO $$
DECLARE
  subscription_rec RECORD;
  plan_max_users INTEGER;
BEGIN
  FOR subscription_rec IN 
    SELECT cs.*, sp.max_users as plan_max_users
    FROM public.company_subscriptions cs
    JOIN public.subscription_plans sp ON cs.plan_id = sp.id
  LOOP
    -- Determinar limite de usuários (customizado ou do plano)
    plan_max_users := COALESCE(subscription_rec.custom_max_users, subscription_rec.plan_max_users, 50);
    
    -- Atualizar limites da empresa
    INSERT INTO public.company_limits (
      company_id, 
      max_users, 
      max_admins, 
      max_managers, 
      max_supervisors
    ) VALUES (
      subscription_rec.company_id,
      plan_max_users,
      1,
      GREATEST(1, plan_max_users / 10),
      GREATEST(1, plan_max_users / 5)
    )
    ON CONFLICT (company_id) 
    DO UPDATE SET
      max_users = plan_max_users,
      max_managers = GREATEST(1, plan_max_users / 10),
      max_supervisors = GREATEST(1, plan_max_users / 5),
      updated_at = NOW();
  END LOOP;
END $$;