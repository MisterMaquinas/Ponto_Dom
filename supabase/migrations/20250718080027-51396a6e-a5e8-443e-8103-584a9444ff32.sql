...
-- Habilitar RLS na view (não aplicável diretamente, mas criamos políticas nas tabelas base)

-- Criar índices para performance
CREATE INDEX IF NOT EXISTS idx_punch_records_company_timestamp 
ON punch_records USING btree (((
    SELECT company_id FROM users WHERE users.id = punch_records.user_id
)), timestamp DESC);

CREATE INDEX IF NOT EXISTS idx_employee_punch_records_company_timestamp 
ON employee_punch_records USING btree (((
    SELECT company_id FROM branches WHERE branches.id = employee_punch_records.branch_id
)), timestamp DESC);
...
-- Criar função para limpeza automática do cache
